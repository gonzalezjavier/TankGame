package game;

public class GameConstants {
    public static final int START_MENU_SCREEN_WIDTH = 500;
    public static final int START_MENU_SCREEN_HEIGHT = 500;


    public static final int GAME_SCREEN_WIDTH = 1344; //42 columns
    public static final int GAME_SCREEN_HEIGHT = 1024; //32 rows

    public static final int WORLD_WIDTH = 1984;
    public static final int WORLD_HEIGHT = 1984;


    public static final int END_MENU_SCREEN_WIDTH = 500;
    public static final int END_MENU_SCREEN_HEIGHT = 500;
}
